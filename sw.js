...
// v3: API pasa a "network-first" para que los cambios hechos desde /admin se vean al instante
// cuando hay internet, manteniendo fallback offline.
const CACHE_NAME = "colectivo-cache-v3";
...
const API_CACHE_NAME = "colectivo-api-cache-v3";
...
self.addEventListener("fetch", (event) => {
  ...
  if (isPublicApiRequest(url)) {
    event.respondWith((async () => {
      ...
      // Estrategia: NETWORK FIRST (para que se reflejen cambios de /admin en el momento).
      // Si falla (offline), caemos a cache.
      try {
