document.addEventListener('DOMContentLoaded', () => {
    ...
    let lastUserLocation = null; // {lat, lon, ts}
...
function updateUserMarker(lat, lon, { pan = false } = {}) {
    if (!map) return;
    lastUserLocation = { lat, lon, ts: Date.now() };
...
// Si hay un popup abierto de parada lógica, refrescar para mostrar distancia
try {
    if (popupUpdateMarker && popupUpdateMarker.isPopupOpen && popupUpdateMarker.isPopupOpen()) {
        const lk = popupUpdateMarker._logicalKey || '';
        const sn = popupUpdateMarker._logicalStopName || '';
        popupUpdateMarker.setPopupContent(renderLogicalStopPopupHtml(lk, sn, popupUpdateMarker._stopLat, popupUpdateMarker._stopLon));
        wireLogicalStopPopup(popupUpdateMarker, lk, sn, false);
    }
} catch (e) {}
}, () => {
    ...
    function renderLogicalStopPopupHtml(logicalKey, stopName, stopLat, stopLon) {
...
if (typeof stopLat === 'number' && typeof stopLon === 'number') {
    if (lastUserLocation && typeof lastUserLocation.lat === 'number' && typeof lastUserLocation.lon === 'number') {
        const d = haversineMeters(lastUserLocation.lat, lastUserLocation.lon, stopLat, stopLon);
        distanceBlock = `
            <div class="stop-popup__dist">
                <span>Estás a</span>
                <strong>${escapeHtml(formatDistance(d))}</strong>
            </div>
        `;
    } else {
        distanceBlock = `
            <div class="stop-popup__dist">
                <span>Distancia</span>
                <strong class="muted">Activá ubicación</strong>
            </div>
        `;
    }
}
...
<div class="stop-popup__actions">
    <button class="popup-btn" data-action="directions" data-lat="${stopLat}" data-lon="${stopLon}">Cómo llegar</button>
    ${(!lastUserLocation ? `<button class="popup-btn secondary" data-action="locate">Activar ubicación</button>` : ``)}
...
<div class="stop-popup__actions">
    ...
    function startPopupAutoUpdate(marker, logicalKey, stopName, closeOnSelect) {
        ...
        popupUpdateTimer = setInterval(() => {
            ...
            try {
                marker.setPopupContent(renderLogicalStopPopupHtml(logicalKey, stopName, marker._stopLat, marker._stopLon));
...
if (act === 'directions') {
    ...
    let url = '';
    if (lastUserLocation && isFinite(lastUserLocation.lat) && isFinite(lastUserLocation.lon)) {
        url = `https://www.google.com/maps/dir/?api=1&origin=${lastUserLocation.lat},${lastUserLocation.lon}&destination=${lat},${lon}&travelmode=walking`;
    } else {
...
if (act === 'locate') {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
        ...
        updateUserMarker(latU, lonU, { pan: false });
        setHint('Ubicación activada.');

        // refrescar popup (para que muestre distancia)
        marker.setPopupContent(renderLogicalStopPopupHtml(logicalKey, stopName, marker._stopLat, marker._stopLon));
        wireLogicalStopPopup(marker, logicalKey, stopName, closeOnSelect);
    }, () => {
        setHint('No pude obtener tu ubicación (permiso denegado o sin señal).');
...
if (act === 'guardarFav') {
    ...
    function onClickParadaLogica(logicalKey, stop, marker, closeOnSelect) {
        ...
        const html = renderLogicalStopPopupHtml(logicalKey, stop ? stop.name : '', stop ? stop.lat : null, stop ? stop.lon : null);
