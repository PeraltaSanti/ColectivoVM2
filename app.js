document.addEventListener('DOMContentLoaded', () => {
    ...
    // Takeover (fondo + maqueta sponsor)
    const takeoverBg = document.getElementById('takeover-bg');
    const takeoverStaticId = adSlotTop?.dataset?.staticAd || '';
    ...
    async function initAds() {
        ...
        // MODO MAQUETA: si el HTML define un sponsor fijo (data-static-ad),
        // activamos el takeover y NO iniciamos el sistema de rotación.
        if (takeoverStaticId) {
            try {
                document.body.classList.add('has-takeover', 'hide-logo-pill');
                // Permite cambiar solo la carpeta (ads/<id>/takeover.png)
                document.documentElement.style.setProperty('--takeover-bg', `url('ads/${takeoverStaticId}/takeover.png')`);
                // Por si en algún momento querés mostrar/ocultar el overlay sin recargar
                if (takeoverBg) takeoverBg.style.opacity = '1';
            } catch { /* noop */ }
            return;
        }
