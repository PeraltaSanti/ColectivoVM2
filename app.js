document.addEventListener('DOMContentLoaded', () => {
    ...
    // Takeover sponsor (hero + fondo)
    const takeoverBg = document.getElementById('takeover-bg');
    const takeoverHeroSlot = document.getElementById('takeover-hero');
    ...
    function initTakeover() {
        if (!takeoverBg || !takeoverHeroSlot) return;
        const key = takeoverHeroSlot.getAttribute('data-takeover');
        if (!key) return;
        document.body.classList.add('has-takeover');
        const v = 24;
        const base = `ads/${key}/takeover`;
        // Fallback PNG primero, luego image-set (si el navegador lo soporta)
        takeoverBg.style.backgroundImage = `url('${base}.png?v=${v}')`;
        takeoverBg.style.backgroundImage = `image-set(url('${base}.webp?v=${v}') type('image/webp'), url('${base}.png?v=${v}') type('image/png'))`;
    }
