document.addEventListener('DOMContentLoaded', () => {
    ...
    const DEFAULT_APP_SKIN = {
        bg: 'brand/black_tucan_bg.jpg',
        logo: 'brand/black_tucan_logo.png',
        opacity_dark: 0.75,
        opacity_light: 0.28,
        blur_px: 10,
        scale: 1.05
    ...
    function normalizeSkin(raw) {
        ...
        const bg = raw.bg || raw.background || raw.bg_src || raw.bgUrl || null;
        const logo = raw.logo || raw.logo_src || raw.logoUrl || null;
        const opacity_dark = Number(raw.opacity_dark ?? raw.opacityDark ?? raw.bg_opacity_dark);
        const opacity_light = Number(raw.opacity_light ?? raw.opacityLight ?? raw.bg_opacity_light);
        const blur_px = Number(raw.blur_px ?? raw.blur ?? raw.bg_blur_px);
        const scale = Number(raw.scale ?? raw.bg_scale);
        return {
    ...
    function applyBrandSkin(skin) {
        ...
        // Seteamos vars de CSS para el fondo
        ...
        // Logo en header
        if (brandLogo) {
            const src = (skin && skin.logo) ? skin.logo : '';
            if (src) {
                brandLogo.src = src;
