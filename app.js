document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- THEME (colores por línea) ---
    // Negro + color dominante por línea (A rojo, E verde, ESTE amarillo, OESTE azul)
    const THEMES = {
        A: {
            primary: '#b00020',   // rojo oscuro
            accent:  '#ff3b30',   // rojo vivo
            meta:    '#ff3b30'
        },
        E: {
            primary: '#1b5e20',   // verde oscuro
            accent:  '#34c759',   // verde vivo
            meta:    '#34c759'
        },
        ESTE: {
            primary: '#b08900',   // amarillo/dorado oscuro
            accent:  '#ffd60a',   // amarillo vivo
            meta:    '#ffd60a'
        },
        OESTE: {
            primary: '#004aad',   // azul oscuro
            accent:  '#0a84ff',   // azul vivo
            meta:    '#0a84ff'
        }
    };
    ...
    function applyThemeForLine(lineKey) {
        const k = (lineKey || 'A').toUpperCase();
        const theme = THEMES[k] || THEMES.A;

        const root = document.documentElement;
        root.style.setProperty('--primary', theme.primary);
        root.style.setProperty('--accent', theme.accent);
        root.style.setProperty('--primary-rgb', hexToRgbTuple(theme.primary).join(', '));
        root.style.setProperty('--accent-rgb', hexToRgbTuple(theme.accent).join(', '));
        // --highlight-gradient está definido en CSS como: linear-gradient(... var(--primary) ... var(--accent) ...)

        document.body.dataset.linea = k;

        // Color de barra del navegador (PWA)
        const metaTheme = document.querySelector('meta[name="theme-color"]');
        if (metaTheme) metaTheme.setAttribute('content', theme.meta || theme.accent);
    }
    ...
    function bindAdminTriggers() {
        ...
        // Abrir con mantener presionado sobre la etiqueta "Publicidad"
        if (adSlotTop) {
            let holdTimer = null;
            const clear = () => { if (holdTimer) { clearTimeout(holdTimer); holdTimer = null; } };

            adSlotTop.addEventListener('pointerdown', (e) => {
                const badge = e.target && e.target.closest && e.target.closest('.ad-badge');
                if (!badge) return;
                clear();
...
// --- HELPERS ---
    ...
    function syncLineUI() {
        ...
        // Tema por línea (transición suave vía CSS)
    ...
    function getLineColor(lineKey) {
        const k = (lineKey || 'A').toUpperCase();
        return (THEMES[k] || THEMES.A).accent;
    }
